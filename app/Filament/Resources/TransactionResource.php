...
class TransactionResource extends Resource implements HasShieldPermissions
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                // Cari bagian ->actions([...]) dan ganti dua tombol cetakmu dengan ini:

                Tables\Actions\Action::make('SmartPrint')
                    ->label('Cetak')
                    ->icon('heroicon-o-printer')
                    ->color('amber')
                    // Tombol hanya muncul jika Bluetooth aktif ATAU Local aktif
                    ->visible(fn () => \App\Models\Setting::isBluetoothEnabled() || \App\Models\Setting::isLocalEnabled())
                    ->action(function ($record, $livewire) {
                        $order = Transaction::with(['paymentMethod', 'transactionItems.product'])->findOrFail($record->id);

                        // JIKA BLUETOOTH AKTIF: Gunakan method di Livewire (ListTransactions)
                        if (\App\Models\Setting::isBluetoothEnabled()) {
                            $items = $order->transactionItems;
                            $livewire->printStruk($order, $items);
                        }
                        // JIKA HANYA LOKAL AKTIF: Gunakan DirectPrintService
                        else {
